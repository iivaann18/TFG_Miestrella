...
export const createOrder = async (req, res) => {
    try {
        const { items, customerEmail, customerName, customerPhone, shippingAddress, couponCode, subtotal, discount, shippingCost, tax, total, notes } = req.body;
...
export const getOrderById = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.userId;
        const [orders] = await pool.query(`SELECT * FROM orders WHERE id = ? AND (userId = ? OR ? IS NULL)`, [id, userId, userId]);
        if (orders.length === 0) {
...
export const updateOrderStatus = async (req, res) => {
    try {
       ...
       WHERE id = ?`, [status, trackingNumber || null, id]);
