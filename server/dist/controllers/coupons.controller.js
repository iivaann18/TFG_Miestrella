...
export const getAllCoupons = async (req, res) => {
    try {
        const [coupons] = await pool.query(`SELECT 
...
export const getCouponByCode = async (req, res) => {
    try {
        ...
        const [coupons] = await pool.query('SELECT * FROM coupons WHERE code = ? AND isActive = true', [code.toUpperCase()]);
        if (coupons.length === 0) {
...
export const createCoupon = async (req, res) => {
    try {
        const { code, discountType, discountValue, minPurchaseAmount, maxUses, isPermanent, startDate, endDate } = req.body;
        ...
        await pool.query(`INSERT INTO coupons 
...
export const updateCoupon = async (req, res) => {
    try {
        const { id } = req.params;
        const { discountType, discountValue, minPurchaseAmount, maxUses, isPermanent, startDate, endDate } = req.body;
        await pool.query(`UPDATE coupons 
...
export const toggleCoupon = async (req, res) => {
    try {
        ...
        await pool.query('UPDATE coupons SET isActive = ?, updatedAt = NOW() WHERE id = ?', [isActive, id]);
        res.json({
