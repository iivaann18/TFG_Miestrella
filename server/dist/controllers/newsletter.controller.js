    ...
    try {
        ...
        if (existing.length > 0) {
            if (existing[0].isActive) {
                ...
                await pool.query('UPDATE newsletter_subscribers SET isActive = true, unsubscribedAt = NULL WHERE email = ?', [email]);
...
export const unsubscribe = async (req, res) => {
    try {
        ...
        await pool.query('UPDATE newsletter_subscribers SET isActive = false, unsubscribedAt = NOW() WHERE email = ?', [email]);
...
export const getAllSubscribers = async (req, res) => {
    try {
        const [subscribers] = await pool.query('SELECT * FROM newsletter_subscribers ORDER BY subscribedAt DESC');
